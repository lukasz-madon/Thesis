...
\item Lepsza podatno¶æ na zmiany - zespo³y pracuj±ce nad widokiem i modelem mog± pracowaæ niezale¿nie
\end{enumerate}

Natomiast wady to:
\begin{enumerate}%[1)]
\item Z³o¿ono¶æ - aplikacje oparte na tym wzorcu bywaj± rozbudowane, dlatego najczê¶ciej stosuje siê go wobec ¶rednich i du¿ych projektów
...
Dwie kolejne metody s³u¿± do parsowania ³añcuchów tekstowych i zamiany na liczbê zmiennoprzecinkow±. Dlaczego definiowaæ na nowo tak prost± funkcjê, która istnieje w wielu bibliotekach? S± ku temu dwa powody: ró¿ny separator dziesiêtny oraz formatowanie liczb.
...
\item Dla ka¿dego pliku wczytywane s± dane, filtrowane atomy zgodnie z mask±, renderowane i ustawiane s± parametry, które zdefiniowa³ u¿ytkownik
\item Ka¿dy model zapisany jest w pliku w formacie PNG, gdzie jego nazwa odpowiada temperaturze
\end{enumerate}
Poniewa¿ przetwarzanie du¿ej liczby plików mo¿e trwaæ d³u¿sz± chwilê, aplikacja wy¶wietla kolejne modele, tak aby u¿ytkownik wiedzia³, ¿e program nie zawiesi³ dzia³ania. Dodatkowo na ¶rodku ekranu pojawia siê napis ,,Processing Files''.
...
Testowanie tradycyjnie jest jednym z ostatnich etapów w budowaniu aplikacji, jednak nowe metodologie, takie jak Agile, przesuwaj± znaczn± czê¶æ testów do pocz±tkowych etapów.\cite{extest} 
...
Bardzo wa¿nym elementem testowania jest tworzenie testów z uwzglêdnieniem odbiorcy oprogramowania. U¿ytkownicy gry video diametralnie ró¿ni± siê od u¿ytkowników systemu bankowego. 
...
Testowanie oprogramowania mo¿na podzieliæ na kilka kategorii. Jednym z nich jest podzia³ na testy statyczne i dynamiczne. Testy statyczne s± to testy, które nie wymagaj± uruchomienia programu np. Code Review, za¶ testy dynamiczne s± to takie, w których aplikacja jest uruchamiana np. testy jednostkowe.
...
Podczas tworzenia aplikacji do wizualizacji CMA korzysta³em g³ównie z testów GUI, wydajno¶ciowych i regresywnych. Testy GUI maj± na celu sprawdzenie dzia³ania ka¿dego elementu interfejsu. Istotne nie tylko jest poprawne dzia³anie ka¿dego z elementów, ale równie¿ ich zachowanie w okre¶lonej sekwencji. Z tego powodu wraz z komplikacj± GUI liczba testów ro¶nie eksponencjalnie. Mój program posiada tylko kilkana¶cie elementów, dlatego manualne testowanie jest wystarczaj±ce.
...
Ostatnim testem by³a kontrola wizualizacji prawdopodobieñstw obsadzenia. Pos³u¿y³ do tego plik test.chmc, który zawiera³ kombinacje obsadzeñ ze zmian± od ca³kowitego Al do ca³kowitego Mg poprzez obsadzenia mieszane. Generuje to w sumie 20 atomów (rysunek \ref{fig:test}).
...
Na rysunku \ref{fig:test} widaæ, ¿e przej¶cia w palecie s± g³adkie oraz kolor, zgodnie z oczekiwaniami, przechodzi od czerwonego do niebieskiego z domieszk± zielonej barwy.